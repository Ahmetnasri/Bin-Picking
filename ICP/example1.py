...
def draw_coordinate(img, z):
    ...
    #point = (int(x),int(y))
    img = cv.line(img, point, point2, (0,0,255), 5)
    img = cv.line(img, point, point3, (0,0,255), 5)

    img = cv.line(img, point, point4, (255,0,0), 5)
    img = cv.line(img, point, point6, (255,0,0), 5)
    #cv.imshow('img',img)
    #cv.waitKey(0)
    return img
...
def get3d(img):
    ...
    a = draw_coordinate(i,guess_world_to_object_vector[5][0])
    return a
